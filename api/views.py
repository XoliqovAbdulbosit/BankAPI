...
from .models import Card, Code, Contact, Transaction
    ...
    def get_queryset(self):
        ...
        if phone_number:
            ...
            queryset = queryset.filter(
                Q(sender__phone_number=phone_number)
                | Q(receiver__phone_number=phone_number)
            )
        if card:
            queryset = queryset.filter(
                Q(sender__number=card) | Q(receiver__number=card)
            )
        return queryset
    ...
    if "message" in update:
        ...
        if msg == "/start":
            ...
            Contact.objects.update_or_create(
                chat_id=chat_id, defaults={"phone_number": msg}
            )
...
def add_card(request):
    ...
    try:
        ...
        if data.get("code") == code.code and card.phone_number == data.get(
            "phone_number"
        ):
            code.delete()
            return JsonResponse(
                {"status": "Success", "card": CardSerializer(card).data}
            )
        else:
            return JsonResponse({"status": "Fail"})
    except ObjectDoesNotExist:
...
def transaction(request):
    ...
    try:
        ...
        return JsonResponse(
            {"status": "Sender or Receiver Card not found or Invalid amount"}
        )
